...
class TypologyContainer extends Component {
    ...
    create() {
        ...
        axios.post(router.route('typologies.store'), payload)
            .then((response) => {
                if (response.data.success) {
                    _this.onSaveSuccess(response.data);

                    setTimeout(function () {
                        window.location.href = router.route('typologies.show', {
                            'typology?' : response.data.typology.id
                        });
                    }, 1500);
                }
            })
    ...
    delete() {
        ...
        var route = router.route('typologies.delete', {
            'typology?' : this.state.typology.id
        });
        ...
        axios.delete(route)
            .then((response) => {
                if (response.data.success) {
                    _this.onSaveSuccess(response.data);
                    window.location.href = router.route('typologies');
                }
            })
            .catch((error) => {
                if (error.response) {
    ...
    update() {
        var _this = this;
        var payload = this.getFormData();
        var route = router.route('typologies.update', {
            'typology?' : this.state.typology.id
        });
