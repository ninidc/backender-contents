<div id="{{$id}}" class="dropzone-{{$size}} dropzone-{{$name}}">
	...
	<div class="dropzone-container" @if($image != null) style="display:none;" @endif>
		<div id="image-dropzone" class="dropzone image-container">
            ...
            <div class="message">
    			<p>{{Lang::get('backender::fields.drag_file')}}<br><span class="link"><i class="fa fa-upload"></i> &nbsp; {{Lang::get('backender::fields.upload_file')}}</span></p>
					@if(isset($sizeText))
						<p>( {{Lang::get('backender::fields.size')}} {{$sizeText}} )</p>
					@endif
    		</div>
    	</div>
	</div>
	...
	<div class="image-container uploaded" @if($image == null) style="display:none;" @endif>
		...
		<div class="actions">
			<a href="" class="btn btn-table" id="remove-picture"><i class="fa fa-trash"></i> &nbsp; {{Lang::get('backender::fields.delete')}} </a>
		</div>
	</div>
